 ...
 * \return GOLLE_OK if successful. GOLLE_ERROR if state or set is
 * NULL. GOLLE_EEMPTY if the set is empty. GOLLE_ETOOFEW if a replacement
 * would result in too few elements being available for all peers. GOLLE_EEXISTS
 * if there are any duplicate elements in the list.
 */
 ...
 *
 * Note, every peer must share the same prime.
 */
GOLLE_EXTERN golle_error golle_prime_allocate (golle_t *state, size_t bits);
 ...
 * \param rand The encrypted selection bytes from the peer.
 ...
 * \return GOLLE_ERROR if any parameter is NULL. GOLLE_EMPTY if a private key
