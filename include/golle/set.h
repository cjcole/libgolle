 ...
 * \return GOLLE_OK if the item was inserted. GOLLE_EEXISTS if the set
 * already contains the item. GOLLE_EMEM if the set did not contain enoug
 * space to insert the item, and no more could be allocated. GOLLE_ERROR if
 * set is NULL.
 *
 ...
 * \return GOLLE_OK if the item was removed. GOLLE_ENOTFOUND if the
 * set didn't contain the item based on the given comp function. GOLLE_ERROR
 * if set is NULL.
 * 
